...
public interface ProductRepository extends JpaRepository<Products, Long> {
    ...
    @Query("SELECT p FROM Products p LEFT JOIN FETCH p.categories LEFT JOIN FETCH p.user")
    ...
    @Query("SELECT p FROM Products p LEFT JOIN FETCH p.categories LEFT JOIN FETCH p.user WHERE p.user.id = ?1")
